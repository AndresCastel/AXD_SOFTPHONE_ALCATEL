...
namespace Axede.Mensajes
{
    ...
    public class AdministradorMensaje
    {
        ...
        private ConfiguracionMensaje GetConfiguration()
        {
            ...
            if (ConfigurationSettings.AppSettings["XmlMensajes"].Contains(":"))
            {
                sPathConfig = string.Format("{0}", ConfigurationSettings.AppSettings["XmlMensajes"]);
            }
            else
            {
                sPathConfig = string.Format("{0}{1}", AppDomain.CurrentDomain.BaseDirectory, ConfigurationSettings.AppSettings["XmlMensajes"]);
            }
